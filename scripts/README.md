...
# process_multiple_shp_CNES

## Purpose
 _This is a python wrapper for processing multiple shapefiles through the
...
### This wrapper follows these steps:
    ...
    been created). The passplan.txt file specifies the date of each SWOT 
...
```
Usage: process_multiple_shp_CNES.py 
```
Inputs / Parameters
-
All these parameters are to be modified in the script :

* ___testcase___ : Name of test case folder (containing the watermask shapefiles folder)

Shapefile filename date in datetime convention. Example: If shapefile filename is *"20170923_Der_S2_SVM_SERTIT.shp"*, then:

* ___modeltimefmt___ = '%Y%m%d' 

* ___shapefile_basename___ : '_Der_S2_SVM_SERTIT'

Specify filepaths :  

* ___orbit_rdf_path___ : path to *parameter_orbit.rdf* file.

* ___sisimp_rdf_path___ : path to *parameter_sisimp.rdf* file.

* ___lake_tile_cfg_path___ : path to *multi_lake_tile_command.cfg* file.

* ___planpath___ : path to the passplan.txt file.

* ___maskpath___ : path to the watermask shapefiles folder.

Outputs
-
Passplan.txt files generated by the select_orbit step.

* ___output_orbit_path___ : path to the orbits output folder.

Pixels cloud (PixC) generated by the SISIMP simulator from water masks.

* __output_sisimp_path__ : path to the SISIMP output folder.

The path to lake_tile's outputs is defined in the parameter file __lake_tile_cfg_path__.