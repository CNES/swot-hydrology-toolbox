...
* __ttt__ is the tile reference, on 3 digits
...
As the LakeSP products are split per pass and continent, LakeTile products refer to a continent. This continent split is set by the __CONTINENT_FILE__ key in the command file.

This file must be a polygon shapefile, with at least an attribute named *MAJ_BAS* equal to:
* 1 for North America
* 2 for Central Ameria + Caribbean island
* 3 for South America
* 4 for Europe
* 5 for East Asia
* 6 for West Asia
* 7 for Africa
* 8 for Oceania (including Australia)
* 9 for Antarctica

NB: if the __CONTINENT_FILE__ parameter is not set, the LakeTile product is not linked to a continent.
...
#Pass number = <int>
# Tile reference (ttt) and swath (s) in string format ("ttts"); if empty, deal with all PixC files of cycle "Cycle" and pass "Pass" in PIXC directory
#Tile ref = <ttts>
```
...
* __Tile ref__ is the tile reference (ttt) and swath (s) in string format ("ttts"); if empty, deal with all PixC files of cycle and pass numbers above in PIXC directory
