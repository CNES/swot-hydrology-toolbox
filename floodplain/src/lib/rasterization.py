...
# cars import
from lib import filtering, projection
from lib import constants as cst
...
def create_raster_dataset(
    ...
    # Only with numpy version > 1.17
    # ~ hgt = np.nan_to_num(raster[..., 0], nan=hgt_no_data)
    # Fix for numpy 1.15.4
    hgt = np.where(np.isnan(raster[..., 0]), hgt_no_data, raster[..., 0])
    ...
    if raster.shape[-1] > 1:  # rasterizer produced color output
        ...
        # Fix for numpy 1.15.4
        clr = np.where(np.isnan(np.rollaxis(raster[:, :, 1:], 2)), color_no_data, np.rollaxis(raster[:, :, 1:], 2))
