...
class myRdfReader(object):
    ...
    def getValueList(self, IN_key):
        '''
        Get the list associated to the key named IN_key
        ...
        '''
        if (IN_key in self.parameters ):
            return list(self.parameters.get(IN_key).split(" "))
        else:
            for key in self.parameters.keys():
                if ( IN_key in key ):
                    return list(self.parameters.get(key).split(" "))
            raise ValueError( "[my_rdf_file/getValue] Unkown key = %s" % ( IN_key ))
